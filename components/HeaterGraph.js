...
const HeaterGraph = () => {
  ...
  // Define the onMessageArrived callback
    ...
    const onMessageArrived = useCallback((message) => {
      ...
      if (message.destinationName === "heaterSide") {
  ...
  useFocusEffect(
    useCallback(() => {
      console.log("heaterlSide screen is focused");
  
      const fetchHeaterSideData = async () => {
        try {
      ...
      if (!mqttServiceRef.current) {
        ...
        mqtt.connect("Tortoise", "Hea1951Ter", {
          onSuccess: () => {
            ...
            mqtt.client.subscribe("heaterSide");
          ...
          onFailure: (error) => {
            ...
            console.error("heaterSide MQTT connection failed:", error);
          },
        });
      ...
      return () => {
        console.log("heaterSideis unfocused, cleaning up...");
        if (mqttServiceRef.current) {

...
const OutSideGraph = () => {
  ...
  const onMessageArrived = useCallback((message) => {
    ...
    useFocusEffect(
      useCallback(() => {
        console.log("outSide screen is focused");
        ...
        if (!mqttServiceRef.current) {
          ...
          mqtt.connect("Tortoise", "Hea1951Ter", {
            ...
            onFailure: (error) => {
              ...
              console.error("outSide MQTT connection failed:", error);
            },
          });
        ...
        return () => {
          console.log("outSideis unfocused, cleaning up...");
          if (mqttServiceRef.current) {
